...
(N in this question) contains exactly an integer number of periods of the sinusoid. 

For example, if f = 100 Hz and fs = 1000 Hz, one period of the sinusoid has 10 samples. 

Then given a signal of length M = 25 samples, there are 2.5 periods in it. 

The minimum zero-padding length here would be 5 samples (0.5 period), so that the DFT size N = 30 
corresponds to 3 periods of a sinusoid of frequency f=100 Hz.
...
def optimalZeropad(x, fs, f):
    ...
    """
    ## Your code here
    samplesPP = fs / f                      # samples per period
    M = len( x )                            # M
    periods = M / samplesPP                 # periods in M
    objective = np.ceil( periods )          # greater integer near periods
    diference = objective - periods         # diference for a full period
    N = M + diference * samplesPP           # M + periods that remain
    X = fft(x,N)                            # computes the ffd with the remaining 0's
    mX = 20*np.log10(abs(X[:(N/2)+1]))      # dB conversion
    return mX