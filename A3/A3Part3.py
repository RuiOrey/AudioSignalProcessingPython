...
zero phase windowed signal and use the symmetry of the computed DFT to test if the innput signal x 
...
def testRealEven(x):
    ...
    for i in range (0, M/2 ) :
        dftbuffer[i + 1] = x[ M /2 + 1 + i]
        dftbuffer[ M / 2 + i + 1 ] = x [ i ]
    X = fft(dftbuffer,M)
    Ximag = X.imag
    Xreal = X.real
    for i in range (0, M/2 ) :
        if  Xreal[i + 1] !=  Xreal[ M  - 1 - i]: 
            isEven = False
        if (np.abs(Ximag[i]) > 1e-12) or  np.abs(Ximag[ (M/2) + 1 + i]) > 1e-12:
            isReal = False
    if ( np.abs(Ximag[0]) > 1e-12  ):
        isReal = False
